...
(defn evals
  ...
  ([exp env]
    ...
    (cond
      ...
      (and (list? (first exp))                                                            ;; ((fun (args...) expr) exprs...)
           (= (ffirst exp) 'fun)) (evals (third (first exp))
                                    (new-env (second (first exp))
                                             (map #(evals % env) (rest exp))
                                             env))
      (and (list? (first exp))                                                             ;; ((fex (args...) expr) exprs...)
           (= (ffirst exp) 'fex)) (evals (third (first exp))
                                    (new-env (second (first exp)) (rest exp) env))
      :else (case (first exp)
              fun   exp                                                                    ;; (fun (args...) expr)
              fex   exp                                                                    ;; (fex (args...) expr)
              ...
              (let [fn (evals (first exp) env)]
                (if (fn? fn)
                  (apply (evals (first exp) env) (doall (map #(evals % env) (rest exp))))
                  (evals (apply list (cons (evals (first exp) env) (rest exp))) env))))))) ;; (funcname args...)
